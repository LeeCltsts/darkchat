...
io.on('connection', (socket) => {
	...
	socket.on('userSearching', (userId, interests = []) => {
		// If we are searching, set the user that sent the searching emit to true
		connectedUsers[userId] = {
			"Searching": true,
			"Messages": [],
			"ConnectedClientID": connectedUserId,
		...
		console.log(`User ${userId} is searching for a user with the interests ${connectedUsers[userId]['Interests']}`);
	});
...
async function searchForStranger(ourUser) {
	let usersArray = Object.values(connectedUsers).filter(user => user.Searching && user.UserID !== ourUser.UserID); // Filter only searching users
	for (let user of usersArray) {
		// If we found a stranger with similar interests
        const commonInterests = arraysEqual(user.Interests, ourUser.Interests)
		if (commonInterests.length > 0) {
            ...
            const ourUserSocket = io.sockets.sockets.get(ourUser['ConnectedClientID'])
...
async function binarySearch(arr, target) {
	...
	while (low <= high) {
		...
		if (arr[mid].Searching === target) {
			return arr[mid];
		} else if (arr[mid].ConnectedClientID.localeCompare(target) < 0) { // Use localeCompare for string comparison
			low = mid + 1;
		} else {
			high = mid - 1;
		}
